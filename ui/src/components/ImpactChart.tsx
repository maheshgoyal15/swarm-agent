...
import { ImpactDataPoint } from "@/lib/types";
...
export default function ImpactChart() {
  ...
  useEffect(() => {
    setLoading(true);
    fetchJSON<ImpactDataPoint[]>(`/metrics/impact-timeline?period=${period}`)
      .then((d) => {
        setData(d);
        setLoading(false);
      })
      .catch((err) => {
        console.error("Failed to load impact timeline:", err);
        setLoading(false);
      });
  }, [period]);
        ...
        {loading ? (
          ...
          <ResponsiveContainer width="100%" height={200}>
            ...
            >
              ...
              <XAxis
                dataKey="date"
                tick={{ fill: "#5a6478", fontSize: 10, fontFamily: "var(--font-mono)" }}
                axisLine={false}
                tickLine={false}
                    ...
                    >
                      ...
                      {payload.map((p, i) => (
                        <div key={i} style={{ color: p.color }}>
                          {p.name === "bytes_scanned" ? "Bytes" : "Latency"}: {p.value}
